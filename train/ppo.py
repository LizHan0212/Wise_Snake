"""
Proximal Policy Optimization (PPO) training for the Snake environment.

Run directly for headless training:
    python train/ppo.py

Run with visible pygame window via watch.py:
    python train/watch.py ppo
"""
...
class TrainingLogCallback(BaseCallback):
    """Print episode stats in the same format as q_learning/mcts."""
    ...
    def _on_step(self) -> bool:
        ...
        # If a max episode budget is set (used when rendering via watch_training),
...
class FlatFloatObsWrapper(gym.ObservationWrapper):
    """
    Converts (N,N) int8 grid into flat float32 vector in [0,1] for an MLP policy.
    """
...
def make_env(seed: int, render_mode: str | None = None):
    ...
    # Use the same compact feature encoding as tab_q (plus geometry features) for better learning.
...
def eval_across_seeds(model: PPO, base_seed: int = 1000, n_episodes: int = 30, max_steps: int = 500):
    """Runs evaluation episodes and reports reward + steps."""
    rewards = []
    steps_list = []
    env = make_env(seed=0)
...
def main(render_mode: str | None = None, episodes: int = 3000):
    ...
    model_path = os.path.join(save_dir, "ppo_model.zip")
    ...
    callback.stats_path = os.path.join(save_dir, "ppo_final_stats.txt")
...
def run_training(render: bool = False, total: int = 3000):
    ...
    both for headless runs and when called via watch_training.
    """
